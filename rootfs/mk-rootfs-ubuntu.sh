...
DEB_REPO="http://ports.ubuntu.com/ubuntu-ports"
DEB_DISTRO="jammy"
...
useradd mixtile -m -u 1000 -s /bin/bash || true
usermod -a -G sudo mixtile
usermod -a -G video mixtile
usermod -a -G render mixtile
echo 'root:mixtile' | chpasswd
echo 'mixtile:mixtile' | chpasswd
ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime
echo "Asia/Shanghai" >/etc/timezone
echo "mixtile-ubuntu" >/etc/hostname
echo "127.0.0.1 localhost" >/etc/hosts
echo "127.0.1.1 mixtile-ubuntu" >>/etc/hosts
echo "" >>/etc/hosts
...
apt-get install -fy sudo fakeroot devscripts cmake binfmt-support dh-make \
    dh-exec device-tree-compiler bc cpio parted gdisk dosfstools mtools alsa-utils \
...
usermod -a -G audio mixtile

rm -f /etc/resolv.conf
ln -sf ../run/NetworkManager/resolv.conf /etc/resolv.conf

systemctl enable resize-filesystem
